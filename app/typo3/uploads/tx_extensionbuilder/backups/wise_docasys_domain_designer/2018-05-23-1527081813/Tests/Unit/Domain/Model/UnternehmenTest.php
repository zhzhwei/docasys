...
class UnternehmenTest extends \TYPO3\CMS\Core\Tests\UnitTestCase
{
     ...
     * @test
     */
    public function getRessourcenReturnsInitialValueForRessource()
    {
        $newObjectStorage = new \TYPO3\CMS\Extbase\Persistence\ObjectStorage();
        self::assertEquals(
            $newObjectStorage,
            $this->subject->getRessourcen()
        );
     ...
     * @test
     */
    public function setRessourcenForObjectStorageContainingRessourceSetsRessourcen()
    {
        $ressourcen = new \Wise\WiseDocasysDomainDesigner\Domain\Model\Ressource();
        $objectStorageHoldingExactlyOneRessourcen = new \TYPO3\CMS\Extbase\Persistence\ObjectStorage();
        $objectStorageHoldingExactlyOneRessourcen->attach($ressourcen);
        $this->subject->setRessourcen($objectStorageHoldingExactlyOneRessourcen);

        self::assertAttributeEquals(
            $objectStorageHoldingExactlyOneRessourcen,
            'ressourcen',
            $this->subject
        );
     ...
     * @test
     */
    public function addRessourcenToObjectStorageHoldingRessourcen()
    {
        $ressourcen = new \Wise\WiseDocasysDomainDesigner\Domain\Model\Ressource();
        $ressourcenObjectStorageMock = $this->getMockBuilder(\TYPO3\CMS\Extbase\Persistence\ObjectStorage::class)
            ->setMethods(['attach'])
            ->disableOriginalConstructor()
            ->getMock();

        $ressourcenObjectStorageMock->expects(self::once())->method('attach')->with(self::equalTo($ressourcen));
        $this->inject($this->subject, 'ressourcen', $ressourcenObjectStorageMock);

        $this->subject->addRessourcen($ressourcen);
    }

    /**
     * @test
     */
    public function removeRessourcenFromObjectStorageHoldingRessourcen()
    {
        $ressourcen = new \Wise\WiseDocasysDomainDesigner\Domain\Model\Ressource();
        $ressourcenObjectStorageMock = $this->getMockBuilder(\TYPO3\CMS\Extbase\Persistence\ObjectStorage::class)
            ->setMethods(['detach'])
        ...
        $ressourcenObjectStorageMock->expects(self::once())->method('detach')->with(self::equalTo($ressourcen));
        $this->inject($this->subject, 'ressourcen', $ressourcenObjectStorageMock);

        $this->subject->removeRessourcen($ressourcen);
