...
class WerkzeugmaschineTest extends \TYPO3\CMS\Core\Tests\UnitTestCase
{
     ...
     * @test
     */
    public function getKinematikReturnsInitialValueForInt()
    {
    }
     ...
     * @test
     */
    public function setKinematikForIntSetsKinematik()
    {
    }
     ...
     * @test
     */
    public function getSchmierungsartReturnsInitialValueForInt()
    {
    }
     ...
     * @test
     */
    public function setSchmierungsartForIntSetsSchmierungsart()
    {
    }
     ...
     * @test
     */
    public function getMesskomponenteReturnsInitialValueForMesskomponente()
    {
        $newObjectStorage = new \TYPO3\CMS\Extbase\Persistence\ObjectStorage();
        self::assertEquals(
            $newObjectStorage,
            $this->subject->getMesskomponente()
        );
     ...
     * @test
     */
    public function setMesskomponenteForObjectStorageContainingMesskomponenteSetsMesskomponente()
    {
        $messkomponente = new \Wise\WiseDocasysDomainDesigner\Domain\Model\Messkomponente();
        $objectStorageHoldingExactlyOneMesskomponente = new \TYPO3\CMS\Extbase\Persistence\ObjectStorage();
        $objectStorageHoldingExactlyOneMesskomponente->attach($messkomponente);
        $this->subject->setMesskomponente($objectStorageHoldingExactlyOneMesskomponente);

        self::assertAttributeEquals(
            $objectStorageHoldingExactlyOneMesskomponente,
            'messkomponente',
            $this->subject
        );
     ...
     * @test
     */
    public function addMesskomponenteToObjectStorageHoldingMesskomponente()
    {
        $messkomponente = new \Wise\WiseDocasysDomainDesigner\Domain\Model\Messkomponente();
        $messkomponenteObjectStorageMock = $this->getMockBuilder(\TYPO3\CMS\Extbase\Persistence\ObjectStorage::class)
            ->setMethods(['attach'])
            ->disableOriginalConstructor()
            ->getMock();

        $messkomponenteObjectStorageMock->expects(self::once())->method('attach')->with(self::equalTo($messkomponente));
        $this->inject($this->subject, 'messkomponente', $messkomponenteObjectStorageMock);

        $this->subject->addMesskomponente($messkomponente);
    }

    /**
     * @test
     */
    public function removeMesskomponenteFromObjectStorageHoldingMesskomponente()
    {
        $messkomponente = new \Wise\WiseDocasysDomainDesigner\Domain\Model\Messkomponente();
        $messkomponenteObjectStorageMock = $this->getMockBuilder(\TYPO3\CMS\Extbase\Persistence\ObjectStorage::class)
            ->setMethods(['detach'])
        ...
        $messkomponenteObjectStorageMock->expects(self::once())->method('detach')->with(self::equalTo($messkomponente));
        $this->inject($this->subject, 'messkomponente', $messkomponenteObjectStorageMock);

        $this->subject->removeMesskomponente($messkomponente);
